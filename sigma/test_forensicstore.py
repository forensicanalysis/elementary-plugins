...
class TestFullTextSearch(unittest.TestCase):
    ...
    def test_regular_queries(self):
        ...
        detection = {"selection": {"fieldname": "test1"},
                     "condition": "selection"}
        expected_result = 'select json from {} where json_extract(json, \'$.type\') = \'eventlog\' and json_extract(json, \'$.fieldname\') = "test1"'.format(
            self.table)
        self.validate(detection, expected_result)

        detection = {"selection": {"fieldname": 4}, "condition": "selection"}
        expected_result = 'select json from {} where json_extract(json, \'$.type\') = \'eventlog\' and json_extract(json, \'$.fieldname\') = "4"'.format(
            self.table)
        self.validate(detection, expected_result)

        detection = {"selection": {"fieldname": [
            "test1", "test2"]}, "condition": "selection"}
        expected_result = 'select json from {} where json_extract(json, \'$.type\') = \'eventlog\' and json_extract(json, \'$.fieldname\') in ("test1", "test2")'.format(
            self.table)
        self.validate(detection, expected_result)

        detection = {"selection": {
            "fieldname": [3, 4]}, "condition": "selection"}
        expected_result = 'select json from {} where json_extract(json, \'$.type\') = \'eventlog\' and json_extract(json, \'$.fieldname\') in ("3", "4")'.format(
            self.table)
        self.validate(detection, expected_result)

        detection = {"selection": {"fieldname1": "test1", "fieldname2": [
            "test2", "test3"]}, "condition": "selection"}
        expected_result = 'select json from {} where json_extract(json, \'$.type\') = \'eventlog\' and (json_extract(json, \'$.fieldname1\') = "test1" and json_extract(json, \'$.fieldname2\') in ("test2", "test3"))'.format(
            self.table)
        self.validate(detection, expected_result)

        detection = {"selection": {"fieldname": "test1"}, "filter": {
            "fieldname2": "whatever"}, "condition": "selection and filter"}
        expected_result = 'select json from {} where json_extract(json, \'$.type\') = \'eventlog\' and (json_extract(json, \'$.fieldname\') = "test1" and json_extract(json, \'$.fieldname2\') = "whatever")'.format(
            self.table)
        self.validate(detection, expected_result)

        detection = {"selection": {"fieldname": "test1"}, "filter": {
            "fieldname2": "whatever"}, "condition": "selection or filter"}
        expected_result = 'select json from {} where json_extract(json, \'$.type\') = \'eventlog\' and (json_extract(json, \'$.fieldname\') = "test1" or json_extract(json, \'$.fieldname2\') = "whatever")'.format(
            self.table)
        self.validate(detection, expected_result)

        detection = {"selection": {"fieldname": "test1"}, "filter": {
            "fieldname2": "whatever"}, "condition": "selection and not filter"}
        expected_result = 'select json from {} where json_extract(json, \'$.type\') = \'eventlog\' and (json_extract(json, \'$.fieldname\') = "test1" and not (json_extract(json, \'$.fieldname2\') = "whatever"))'.format(
            self.table)
        self.validate(detection, expected_result)

        detection = {"selection": {"fieldname1": "test1"}, "filter": {
            "fieldname2": "test2"}, "condition": "1 of them"}
        expected_result = 'select json from {} where json_extract(json, \'$.type\') = \'eventlog\' and (json_extract(json, \'$.fieldname1\') = "test1" or json_extract(json, \'$.fieldname2\') = "test2")'.format(
            self.table)
        self.validate(detection, expected_result)

        detection = {"selection": {"fieldname1": "test1"}, "filter": {
            "fieldname2": "test2"}, "condition": "all of them"}
        expected_result = 'select json from {} where json_extract(json, \'$.type\') = \'eventlog\' and (json_extract(json, \'$.fieldname1\') = "test1" and json_extract(json, \'$.fieldname2\') = "test2")'.format(
            self.table)
        self.validate(detection, expected_result)
    ...
    def test_modifiers(self):
        ...
        detection = {"selection": {"fieldname|contains": "test"},
                     "condition": "selection"}
        expected_result = 'select json from {} where json_extract(json, \'$.type\') = \'eventlog\' and json_extract(json, \'$.fieldname\') like "%test%" escape \'\\\''.format(
            self.table)
        self.validate(detection, expected_result)
        ...
        detection = {"selection": {"fieldname|all": [
            "test1", "test2"]}, "condition": "selection"}
        expected_result = 'select json from {} where json_extract(json, \'$.type\') = \'eventlog\' and (json_extract(json, \'$.fieldname\') = "test1" and json_extract(json, \'$.fieldname\') = "test2")'.format(
            self.table)
        self.validate(detection, expected_result)
    ...
    def test_aggregations(self):
        ...
        inner_query = 'select min(fieldname2) as agg from {} where json_extract(json, \'$.fieldname1\') = "test"'.format(
            self.table)
        expected_result = 'select json from ({}) where agg > 5'.format(inner_query)
        self.validate(detection, expected_result)
        ...
        inner_query = 'select max(fieldname2) as agg from {} where json_extract(json, \'$.fieldname1\') = "test"'.format(
            self.table)
        expected_result = 'select json from ({}) where agg > 5'.format(inner_query)
        self.validate(detection, expected_result)
        ...
        inner_query = 'select avg(fieldname2) as agg from {} where json_extract(json, \'$.fieldname1\') = "test"'.format(
            self.table)
        expected_result = 'select json from ({}) where agg > 5'.format(inner_query)
        self.validate(detection, expected_result)
        ...
        inner_query = 'select sum(fieldname2) as agg from {} where json_extract(json, \'$.fieldname1\') = "test"'.format(
            self.table)
        expected_result = 'select json from ({}) where agg > 5'.format(inner_query)
        self.validate(detection, expected_result)
        ...
        inner_query = 'select sum(fieldname2) as agg from {} where json_extract(json, \'$.fieldname1\') = "test" group by fieldname3'.format(
            self.table)
        expected_result = 'select json from ({}) where agg == 5'.format(inner_query)
        self.validate(detection, expected_result)
        ...
        inner_query = 'select sum(fieldname2) as agg from {} where (json_extract(json, \'$.fieldname1\') = "test" or json_extract(json, \'$.fieldname2\') = "tessst")'.format(
            self.table)
        expected_result = 'select json from ({}) where agg == 5'.format(inner_query)
        self.validate(detection, expected_result)
    ...
    def test_wildcards(self):
        ...
        detection = {"selection": {"fieldname": "test*"},
                     "condition": "selection"}
        expected_result = 'select json from {} where json_extract(json, \'$.type\') = \'eventlog\' and json_extract(json, \'$.fieldname\') like '.format(
        ...
        expected_result = 'select json from {} where json_extract(json, \'$.type\') = \'eventlog\' and ({} or {})'.format(
        ...
        opt1 = 'json_extract(json, \'$.fieldname\') like ' + r'"test%"' + r" escape '\'"
        opt2 = 'json_extract(json, \'$.fieldname\') like ' + r'"%test"' + r" escape '\'"
        expected_result = 'select json from {} where json_extract(json, \'$.type\') = \'eventlog\' and ({} and {})'.format(
            self.table, opt1, opt2)
        self.validate(detection, expected_result)
    ...
    def test_fieldname_mapping(self):
        detection = {"selection": {"fieldname": "test1"},
                     "condition": "selection"}
        expected_result = 'select json from {} where json_extract(json, \'$.type\') = \'eventlog\' and json_extract(json, \'$.mapped_fieldname\') = "test1"'.format(
            self.table)
        ...
        with patch("yaml.safe_load_all", return_value=[self.basic_rule]):
            ...
            backend = ForensicStoreBackend(config, self.table)
